...
import { getAnnouncements, deleteAnnouncementById } from 'apiSdk/announcements';
import { AnnouncementInterface } from 'interfaces/announcement';

type ColumnType = ColumnDef<AnnouncementInterface, unknown>;
...
export function AnnouncementListPage(props: AnnouncementListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AnnouncementInterface>>(
    () => `/announcements?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AnnouncementInterface) => {
    if (hasAccess('announcement', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/announcements/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/announcements/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('announcement', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/announcements/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('announcement', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="announcement" />
        </Flex>

        {hasAccess('announcement', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/announcements/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
