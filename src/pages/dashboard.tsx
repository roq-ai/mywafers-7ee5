...
import { getBirthdays } from 'apiSdk/birthdays';
import { BirthdayInterface } from 'interfaces/birthday';
import { getAnnouncements } from 'apiSdk/announcements';
...
function DashboardPage() {
  ...
  const birthdayStat = useSWR<PaginatedInterface<BirthdayInterface>>(
    () => `/birthdays`,
    () => getBirthdays({ limit: 1, offset: 0 }),
  );

  const announcementStat = useSWR<PaginatedInterface<AnnouncementInterface>>(
    () => `/announcements`,
    () => getAnnouncements({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Birthdays',
      result: birthdayStat?.data?.totalCount,
      isLoading: birthdayStat.isLoading,
    },
    {
      title: 'Announcements',
      result: announcementStat?.data?.totalCount,
      isLoading: announcementStat.isLoading,
    },
    {
      title: 'Rewards',
